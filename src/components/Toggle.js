...
const Toggle = styled.div`
	...
	border-radius: 1.5em;
	cursor: pointer;
	box-shadow: 0 0.2em hsl(0, 0%, 50%) inset;
	animation: 0.25s linear ${fade_out} forwards;
	&:hover {
		animation: 0.25s linear ${fade_in} forwards;
		box-shadow: 0 0 1em 1.5em
			${(props) => (props.isDarkModeOn ? '#1a1a1a' : '#fafafa')};
	}
...
const Button = styled.div`
	...
	background-color: ${(props) =>
		props.isDarkModeOn ? '#1a1a1a' : 'whitesmoke'};
	transition: 0.1s linear;
	box-shadow: 0 0.2em hsl(0, 0%, 50%);
	${(props) =>
		props.isDarkModeOn &&
		css`
